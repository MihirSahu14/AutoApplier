...
export function Outreach({ jobId }: { jobId: number }) {
  ...
  const find = useMutation({
    ...
    onSuccess: (r) => {
      ...
      } else if (r.total_returned === 0 || r.total_returned === undefined) {
    ...
    <div className="bg-white border border-slate-200 rounded shadow-sm p-5 mt-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          onClick={() => find.mutate()}
          className="px-3 py-1.5 bg-fuchsia-600 text-white rounded text-sm hover:bg-fuchsia-700 disabled:opacity-50"
        >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="text-sm">
          <div className="font-medium">
            ...
            {c.confidence !== null && (
              <span className="ml-2 text-xs text-slate-400 font-normal">conf {c.confidence}</span>
            )}
            {c.sent_at && (
              <span className="ml-2 text-xs text-emerald-700 bg-emerald-50 border border-emerald-200 rounded px-1.5">
                sent
              </span>
            )}
          </div>
          <div className="text-slate-500">{c.title || ""}</div>
          <div className="text-slate-700 font-mono text-xs mt-0.5">{c.email}</div>
          {c.linkedin && (
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 grid gap-2">
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-slate-200 text-slate-800 rounded text-xs hover:bg-slate-300"
            >
              Copy body
            </button>
            <button
